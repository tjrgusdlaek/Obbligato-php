...
$sql = "select * from `User` where `User_email`='$Writer'";
...
$sql = "update `User` set `User_deposit_money` ='$return_deposit' where `User_email`='$Writer'";
$result = mysqli_query($conn, $sql);
...
$sql = "update `Apply_Request` set `Status` ='계약취소' where `Post_index`='$get_Post_index' and `Writer`='$Writer' and `Post_title` ='$get_Title' and `Applier` = '$get_Email'";
$result = mysqli_query($conn, $sql);
$response["sql"] = $sql;

$sql = "delete from `Final_contract` where `id` = '$Index'";
$result = mysqli_query($conn, $sql);
